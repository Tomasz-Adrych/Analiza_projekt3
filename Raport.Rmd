...
Ten zestaw danych zapewnia szczegółowy przegląd rutynowych ćwiczeń członków siłowni, atrybutów fizycznych i wskaźników sprawności. Zawiera on 973 obserwacji, w tym kluczowe wskaźniki wydajności, takie jak tętno, spalone kalorie i czas trwania treningu. Każdy wpis obejmuje również dane demograficzne i poziomy doświadczenia, co pozwala na kompleksową analizę wzorców sprawności, postępów sportowców i trendów zdrowotnych.
...
W pierwszym kroku zostały zidentyfikowane wartości brakujące. W wykorzystywanym zbiorze danych, braki zlokalizowano w kolumnach kolejno: Age, Workout Type oraz BMI (Body Mass Index). Do zobrazowania braków użyto pakietu VIS.
...
Drugi etap opierał się na zwizualizowaniu wartości odstających. Za pomocą wykresów pudełkowych znaleziono wartości odstające w kolumnach Weight oraz Calories_Burned. Wartości te nie budzą większych zastrzeżeń, gdyż odchylenia w tego typu danych są normalne. Wykresy pudełkowe dla pozostałych zmiennych nie wykazują wartości odstających.
...
Trzecim etapem projektu było opracowanie reguł walidacyjnych. Zostały one stworzone dla każdej zmiennej wchodzącej w skład zbioru danych. W poniższym chunku opisano je oraz przedstawiono w tabeli. Końcowa tabela zawiera informację, czy reguły w poszczególnych zmiennych zostały spełnione. Wszystkie zmienne oprócz kolumny Avg_BPM spełniły wskazane reguły. Jednak w tym przypadku nie ma powodów do zmieniania danych, gdyż tętno ludzkie charakteryzuje się dużymi odchyleniami i może różnić się w zależności od innych zmiennych takich jak: masa ciała, intensywność treningu etc.
...
Czwarty etap raportu opierał się na uzupełnieniu wartości brakujących w trzech kolumnach wskazanych w w etapie drugim. Do uzupełnienia braków w kolumnie Age, użyto metody RPART (Recursive Partitioning and Regression Trees). W przypadku kolumny BMI braki uzupełniono za pomocą wzoru: Waga / Wzrost\^2. Dla zmiennej Workout_Type wybrano metodę MICE (Multivariate Imputation by Chained Equations).
...
#Dane do obliczeń; bez zmiennych Age i BMI zawierających braki
...
Piąty etap zawiera wizualizacje najistotniejszych danych z obszaru wydajności oraz efektywności treningów. Do opracowania wizualizacji użyto pakietów plotly oraz ggplot2.
...
W ramach szóstego etapu sporządzono w tabeli statystyki opisowe dla zmiennych ilościowych. Obliczono między innymi: średnią, odchylenie standardowe, kwantyle, wariancję, współczynnik zmienności, skośność oraz kurtozę. Ponadto obliczono korelacje pomiędzy zmiennymi oraz sporządzono macierz,aby czytelniej przedstawić wyniki.
...
# Model liniowy regresji wielorakiej objaśniający zależności między spalonymi kaloriami a czasem trwania trenigu, BMI, procentem tkanki tłuszczowej oraz średnim tętnem podczas treningu
...
# Ha: Współczynnik przy zmiennej różni się istotnie od zera.
...
#Session_duration_hours: Wzrost czasu trwania sesji o 1 godzinę średnio powoduje wzrost liczby spalonych kalorii o 672.91 kcal ceteris paribus.

#BMI: Wzrost BMI o jedną jednostkę średnio powoduje wzrost liczby spalonych kalorii o 1.9 kcal ceteris paribus.
...
#Avg_BPM: Wzrost średniego tętna podczas treningu o 1 jednostkę średnio powoduje wzrost liczby spalonych kalorii ceteris paribus.

#R^2 = 94%, Zmienne wchodzące w skład modelu objaśniły jego zmienność w 94%. Model jest bardzo dobrze dopasowany.
...
# VIF 5: Wysoka współliniowość. Zmienna jest silnie skorelowana z innymi zmiennymí.

#Wszystkie zmienne w modelu przyjmują wartości nieznacznie większe od 1. Oznacza to, iż mamy do czynienia z brakiem współliniowości, więc zmienne nie są skorelowane z innymi zmiennymi.

# Test normalności rozkładu Shapiro-Wilka

shapiro.test(model$residuals)

#Ho: Reszty pochodzą z populacji o rozkładzie normalnym.
#Ha: Reszty nie pochodzą z populacji o rozkładzie normalnym.

#P-value = 0,2146. Brak podstaw do odrzucenia hipotezy zerowej, reszty pochodzą z populacji o rozkładzie normalnym.
...
# Ha : Występują istotne statystycznie różnice między płcią a BMI.
...
W całym raporcie przeprowadzono kolejno data Wrangling, w tym wstępną analizę danych, czyszczenie danych, zidentyfikowanie wartości brakujących oraz odstających, imputację danych oraz przygotowanie danych do wizualizacji, a następnie wizulizację danych. W kolejnym etapie zostały przedstawione najważniejsze statystyki opisowe wszystkich zmiennych wchodzących w skład naszego zbioru danych. Ponadto stworzono macierz korelacji, aby lepiej zwizualizować zależności pomiędzy poszczególnymi zmiennymi. W ramach ostatniego etapu sporządzono liniowy model regresji wielorakiej, za pomocą którego wyjaśniono, jak na liczbę spalonych kalorii wpływają zmienne: czas trwania sesji treningowej, BMI oraz procent tkanki tłuszczowej. Model okazał się bardzo dobrze dopasowany, a wybrane zmienne istotne statystycznie. Normalność rozkładu reszt sprawdzono za pomocą testu Shapiro-Wilka. Ponadto udowodniono za pomocą testów t, iż typ treningu nie wpływa istotnie na liczbę spalonych kalorii. Pokazano również, że BMI istotnie różni się pomiędzy płciami.
